...
def generate_dataset(num_images=100, line_width=3, seed=0):
    np.random.seed(seed)
    for i in range(num_images):
        ...
        image_path, problem = create_polygon_image(size=500, idx=i, num_sides=num_sides, num_diagonals=num_diagonals, rotation_vertices=rotation_vertices, line_width=line_width, seed=seed)
        ...
        with open(f"polygon_rotation_problems.jsonl", "a") as f:
            json.dump(newjson, f)
            f.write("\n")
...
def main():
    ...
    generate_dataset(num_images=args.num_images, line_width=3, seed=args.seed)
