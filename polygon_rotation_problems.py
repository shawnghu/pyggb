...
def count_connected_components(image_path):
    ...
    for component_label in real_labels:
        ...
        if len(contours) > 0:
            ...
            print(f"Approximated contour for label {component_label} has {len(approx)} points")
        else:
...
def generate_dataset(num_images=100, num_sides=12, line_width=2, seed=0):
    for i in range(num_images):
        print(i)
        num_sides = np.random.randint(6, 13)
        num_diagonals = np.random.randint(1, 4)
        ...
        image_path = create_polygon_image(size=500, idx=i, num_sides=num_sides, num_diagonals=num_diagonals, rotation_vertices=rotation_vertices, line_width=line_width, seed=seed)
        if image_path is None:
            continue
        problem = f'How many connected components are there in the polygon?'
        answer = count_connected_components(image_path)
        newjson["problem"] = problem
        newjson["answer"] = answer
        with open(f"problems.jsonl", "a") as f:
            json.dump(newjson, f)
            f.write("\n")
...
def main():
    ...
    generate_dataset(num_images=1, num_sides=12, line_width=3, seed=args.seed)
